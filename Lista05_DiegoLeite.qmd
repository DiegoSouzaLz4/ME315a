...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES*(1.2) AS "area_ajustada" FROM PRESERVE
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES/3 AS "ÁREA AJUSTADA"
FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50) / 2.0 AS tarifa_revisada
FROM PRESERVE
```
...
```{sql connection=con}
SELECT MIN(PNAME) AS registro_alfabetico
FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS total_tarifas
FROM PRESERVE
WHERE FEE > 0
```
...
SELECT 
  ...
  COUNT(DISTINCT PNAME) AS chaves_unicas,
  COUNT(PNAME) - COUNT(DISTINCT PNAME) AS saldo_duplicatas
FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS estimativa_tarifaria_media_az
FROM PRESERVE
WHERE STATE = 'Arizona'
```
...
SELECT 
  SUM(SALARY) AS total,
  AVG(SALARY) AS media,
  MAX(SALARY) AS teto_salarial,
  MIN(SALARY) AS piso_salarial
FROM EMPLOYEE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS ativos
FROM EMPLOYEE
```
